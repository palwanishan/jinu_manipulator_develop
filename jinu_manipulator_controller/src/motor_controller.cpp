...
Motor_Controller::Motor_Controller(){
  _DEV_MC[3].Board_SetCANIDs(3, 1);   _DEV_MC[3].DIR =  1;    _DEV_MC[3].gear_ratio = 6;  _DEV_MC[3].zeroManualOffset = 0;    _DEV_MC[3].torqueToHex = 1;     
  _DEV_MC[4].Board_SetCANIDs(4, 1);   _DEV_MC[4].DIR = -1;    _DEV_MC[4].gear_ratio = 8;  _DEV_MC[4].zeroManualOffset = 0;  _DEV_MC[4].torqueToHex = -1;   
  _DEV_MC[5].Board_SetCANIDs(5, 1);   _DEV_MC[5].DIR =  1;    _DEV_MC[5].gear_ratio = 6;  _DEV_MC[5].zeroManualOffset = 0;   _DEV_MC[5].torqueToHex = 1;   
...
VectorXd Motor_Controller::GetTheta(){
  for(uint8_t i=0; i<3; i++)  { theta_balanced_rad_0xA1[i] = (65534/2 - _DEV_MC[i+3]._torque_ctrl_encoder_fdback) / tic2radX + _DEV_MC[i+3].zeroManualOffset;    }
